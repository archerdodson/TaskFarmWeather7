...
#  generate the training data: only run once
python generate_data.py lorenz --window_size 10 --n_steps 200 --prediction_length 9 #20000 

#python newcode_predictionl.py lorenz
...
python train_nn.py lorenz SR --scoring_rule SignatureKernel --lr 0.01 --epochs 1000 --ensemble_size 7 --seed 12  --batch_size 1000 --hidden_size_rnn 8
# compute performance metrics and produce individual figures
python predict_test_plot.py lorenz SR --scoring_rule SignatureKernel --lr 0.01  --training_ensemble_size 7 --prediction_ensemble_size 200 --seed 12  --batch_size 1000 --hidden_size_rnn 8

# Ensemble size was 10, now 5
# --- GANs ---
