...
if save_plots:
    ...
    n_time_steps = prediction_length  # Now 15 time steps
    group_size = 5  # Number of time steps per row
    n_groups = (n_time_steps + group_size - 1) // group_size  # Number of groups needed (3 for 15)

    # --- Plot 1: Absolute Values (2 rows per group) ---
    fig, axes = plt.subplots(ncols=group_size, nrows=2*n_groups, 
                             figsize=(25, 4 * n_groups),  # Height scales with groups
    ...
    # Loop through each group of time steps
    for group_idx in range(n_groups):
        start_idx = group_idx * group_size
        end_idx = min((group_idx + 1) * group_size, n_time_steps)
        
        # Row for true realizations (even rows: 0, 2, 4)
        for i in range(start_idx, end_idx):
            col_idx = i - start_idx
            plot_map_ax(da_realizations[i][:, :, 0], 
                        title=f"True t={i}", 
                        ax=axes[2*group_idx, col_idx], 
                        global_projection=global_projection, 
                        vmax=vmax, vmin=vmin)
        
        # Row for predictions (odd rows: 1, 3, 5)
        for i in range(start_idx, end_idx):
            col_idx = i - start_idx
            p = plot_map_ax(da_predictions[i][:, :, 0], 
                            title=f"Pred t={i}", 
                            ax=axes[2*group_idx+1, col_idx], 
                            global_projection=global_projection, 
                            vmax=vmax, vmin=vmin)
    ...
    # --- Plot 2: Differences (1 row per group) ---
    fig, axes = plt.subplots(ncols=group_size, nrows=n_groups, 
                             figsize=(25, 2.5 * n_groups),
                             subplot_kw=dict(projection=ccrs.PlateCarree(), facecolor="gray"))
    ...
    # Plot each group in its own row
    for group_idx in range(n_groups):
        start_idx = group_idx * group_size
        end_idx = min((group_idx + 1) * group_size, n_time_steps)
        ...
        for i in range(start_idx, end_idx):
            ...
            p = plot_map_ax(differences[i][:, :, 0], 
                            ...
                            global_projection=global_projection, 
                            vmin=vmin_diff, vmax=vmax_diff)  # Use diverging colormap
    
    # Add colorbar and save
    fig.subplots_adjust(right=0.9)
    cbar_ax = fig.add_axes([0.92, 0.15, 0.02, 0.7])
    fig.colorbar(p, cax=cbar_ax)
    fig.suptitle(f"Z500, Prediction - Realization, {newdate}", size=20, y=0.97)
