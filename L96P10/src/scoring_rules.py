...
class SignatureKernel(ScoringRule):
    ...
    def estimate_score_batch(self, forecast: TensorType["batch", "ensemble_size", "data_size"],
        ...
        X = torch.cat([zerosX, forecast], dim=1)
        y = torch.cat([zerosY, verification], dim=1)

                #1) Create a time vector, shape [seq_len]
        time_vec = torch.linspace(0, 1, steps=seq_len+1, device=forecast.device)
        # 2) Reshape to match x's batch and sequence dimensions
        #    We want [batch_size, seq_len, 1], so we expand along batch dimension
        time_vec = time_vec.unsqueeze(0).unsqueeze(-1)  # now [1, seq_len, 1]
        ...
        # print("forecast ")
        # batch_size, ensemble_size, data_size = forecast.shape  # b,d,1   #(b,d,) (b,d,1)
        # #In this case should be (b,d,)
        # batch_size1, ensemble_size1 = verification.shape
        # print("forecast " + str(batch_size) +" " + str(ensemble_size) +" " + str(data_size))
        # print("verification " + str(batch_size1) +" " + str(ensemble_size1))
